...
test: ## Run helm-unittest tests
	@if [ -n "$$(find templates/tests -name '*.yaml' 2>/dev/null | head -1)" ]; then \
		echo "Running helm-unittest tests..."; \
		helm unittest .; \
	else \
		echo "No tests found in templates/tests/ — skipping"; \
	fi
...
snapshot-update: ## Update helm-unittest snapshots
	@if [ -n "$$(find templates/tests -name '*.yaml' 2>/dev/null | head -1)" ]; then \
		...
		echo "No tests found in templates/tests/ — nothing to snapshot"; \
	fi
